...
import coda_backward_epilogue_fusion as coda  # noqa: E402
from coda_backward_epilogue_fusion import (  # noqa: E402
...
def _run_coda(ops, x, ws, do_fuse=True):
    ...
    for op, w in zip(ops, ws):
        out = op.apply(out, w)
    loss = out.sum()
    plan = apply_epilogue_fusion(loss.grad_fn, RULES) if do_fuse else None
    loss.backward()
    return plan
...
class TestCodaBwdFusionPlanning(TestCase):
    ...
    def _inputs(self, n):
        ...
        ws = [torch.randn(6, 6, dtype=torch.double, requires_grad=True) for _ in range(n)]
        return x, ws
    ...
    def test_chain_fuses_all_but_tail(self):
        ...
        self.assertEqual(len(plan._fused), 2)
    ...
    def test_no_reference_cycle_leak(self):
        ...
        def run():
            x = torch.randn(4, 6, dtype=torch.double, requires_grad=True)
            ws = [torch.randn(6, 6, dtype=torch.double, requires_grad=True)
                  for _ in range(3)]
        ...
        try:
            ...
            gc.enable()
        self.assertEqual(
            alive, 0, f"{alive} input/weight tensors still alive after backward "
                      f"(held by a reference cycle, not freed by refcount)"
        )

    def test_branching_consumer_bails(self):
        torch.manual_seed(1)
        x = torch.randn(4, 6, dtype=torch.double, requires_grad=True)
        ...
        plan = apply_epilogue_fusion(loss, RULES)
        self.assertEqual(len(plan._fused), 0)
        self.assertTrue(
            all(p.unfused_reason == "branching consumer" for p in plan._bailed)
        )
        loss.backward()
        self.assertEqual(LOG.c["fused_impl"], 0)
        self.assertEqual(LOG.c["epilogue_unfused"], 3)
        ...
        eb = torch.relu(ex @ ew1)
        (torch.relu(eb @ ewa).sum() + torch.relu(eb @ ewb).sum()).backward()
        self.assertEqual(x.grad, ex.grad)
        self.assertEqual(w1.grad, ew1.grad)
        self.assertEqual(wa.grad, ewa.grad)
        self.assertEqual(wb.grad, ewb.grad)
    ...
    def test_unregistered_pair_not_fused(self):
        ...
        self.assertEqual(len(plan._missing_rules), 1)
        loss.backward()
        self.assertEqual(LOG.c["fused_impl"], 0)
    ...
    def test_mixed_chain_fuses_both(self):
        ...
        self.assertIn((MMTanh.main_backward, MMRelu.epilogue_backward), _rule_keys(RULES))
        self.assertIn((MMRelu.main_backward, MMTanh.epilogue_backward), _rule_keys(RULES))

        ops = [MMTanh, MMRelu, MMTanh]
        x, ws = self._inputs(len(ops))
        plan = _run_coda(ops, x, ws)
        self.assertEqual(len(plan._fused), 2)
        producers = {p.producer.cls for p in plan._fused}
    ...
    def test_assert_num_fusions_contract(self):
        ...
        for op, w in zip(ops, ws):
            out = op.apply(out, w)
        plan = apply_epilogue_fusion(out.sum(), RULES)
    ...
    def test_placeholder_rejects_real_ops(self):
        ...
        t = DeferredGradTensor(
            (4, 6), torch.double, torch.device("cpu"),
    ...
    def test_shape_changing_epilogue(self):
        ...
        # (With the old out.view_as(out) carrier this backward would shape-mismatch,
        # since epilogue_backward returns a [.., 2K] grad for the boundary.)
        out.sum().backward()
    ...
    def test_multiple_fusible_inputs_errors(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "deferring more than one grad_input"
        ):
    ...
    def test_epilogue_input_not_first(self):
        ...
        loss = out.sum()
        plan = apply_epilogue_fusion(loss, rules)
        self.assertEqual(len(plan._fused), 1)
        self.assertEqual(plan._fused[0].producer.defer_input_idx, 1)
        loss.backward()
    ...
    def test_expect_num_fusions_kwarg(self):
        ...
        for op, w in zip(ops, ws):
            out = op.apply(out, w)
        plan = apply_epilogue_fusion(out.sum(), RULES, expect_num_fusions=2)
        self.assertEqual(len(plan._fused), 2)
